...
-- ==========================================
-- INTERFACE DE ABAS (AGORA COM 3 ABAS)
-- ==========================================
...
-- O botão original do pulo
local btnToggleMMV = criarBotao("BtnToggleMMV", "Boost Pulo (Speed 100): OFF", nil, Color3.fromRGB(192, 57, 43), containerMM2)

-- NOVA FUNÇÃO: Auto Lançar Facas (No topo!)
local btnAutoFaca = criarBotao("BtnAutoFaca", "Auto Lançar Faca: OFF", nil, Color3.fromRGB(192, 57, 43), containerMM2)

-- Restante dos hacks do MM2
local btnEspMM2 = criarBotao("BtnEspMM2", "ESP Jogadores: OFF", nil, Color3.fromRGB(52, 73, 94), containerMM2)
local btnEspArma = criarBotao("BtnEspArma", "ESP Arma Caída: OFF", nil, Color3.fromRGB(52, 73, 94), containerMM2)
local btnAtirarMurderer = criarBotao("BtnAtirar", "Atirar no Murderer (Xerife)", nil, Color3.fromRGB(231, 76, 60), containerMM2)
local btnTpArma = criarBotao("BtnTpArma", "Teleportar p/ Arma Caída", nil, Color3.fromRGB(46, 204, 113), containerMM2)
...
local btnMatarTodos = criarBotao("BtnMatar", "Matar Todos (Murderer)", nil, Color3.fromRGB(192, 57, 43), containerMM2)
...
-- 1. ESP Jogadores
btnEspMM2.MouseButton1Click:Connect(function()
	espAtivo = not espAtivo
	if espAtivo then
		btnEspMM2.Text = "ESP Jogadores: ON"
		btnEspMM2.BackgroundColor3 = Color3.fromRGB(46, 204, 113)
		...
		btnEspMM2.Text = "ESP Jogadores: OFF"
		btnEspMM2.BackgroundColor3 = Color3.fromRGB(52, 73, 94)
...
-- 2. ESP Arma Caída
btnEspArma.MouseButton1Click:Connect(function()
	espArmaAtivo = not espArmaAtivo
	if espArmaAtivo then
		btnEspArma.Text = "ESP Arma Caída: ON"
		btnEspArma.BackgroundColor3 = Color3.fromRGB(241, 196, 15) 
		...
		btnEspArma.Text = "ESP Arma Caída: OFF"
		btnEspArma.BackgroundColor3 = Color3.fromRGB(52, 73, 94)
...
-- 3. Atirar no Murderer
btnAtirarMurderer.MouseButton1Click:Connect(function()
	if getSheriff() ~= localPlayer then
		btnAtirarMurderer.Text = "Você não tem a arma!"
		task.wait(1.5) btnAtirarMurderer.Text = "Atirar no Murderer (Xerife)" return
	end
	local murd = getMurderer()
	if not murd or not murd.Character or not murd.Character:FindFirstChild("HumanoidRootPart") then
		btnAtirarMurderer.Text = "Nenhum Murderer Achado"
		task.wait(1.5) btnAtirarMurderer.Text = "Atirar no Murderer (Xerife)" return
	end
	if localPlayer.Backpack:FindFirstChild("Gun") then localPlayer.Character.Humanoid:EquipTool(localPlayer.Backpack.Gun) end
	local args = { [1] = 1, [2] = murd.Character.HumanoidRootPart.Position, [3] = "AH2" }
	pcall(function() localPlayer.Character.Gun.KnifeLocal.CreateBeam.RemoteFunction:InvokeServer(unpack(args)) end)
end)
...
btnTpArma.MouseButton1Click:Connect(function()
	local mapa = obterMapa()
	if mapa and mapa:FindFirstChild("GunDrop") then localPlayer.Character:PivotTo(mapa.GunDrop:GetPivot())
	else btnTpArma.Text = "Arma Não Está no Chão" task.wait(1.5) btnTpArma.Text = "Teleportar p/ Arma Caída" end
end)
...
-- 5. Matar Todos
btnMatarTodos.MouseButton1Click:Connect(function()
	if getMurderer() ~= localPlayer then
		btnMatarTodos.Text = "Você não é o Murderer!"
		task.wait(1.5) btnMatarTodos.Text = "Matar Todos (Murderer)" return
	end
	if localPlayer.Backpack:FindFirstChild("Knife") then localPlayer.Character.Humanoid:EquipTool(localPlayer.Backpack.Knife) end
	local myHrp = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
	if not myHrp then return end
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= localPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
			p.Character.HumanoidRootPart.Anchored = true
			p.Character.HumanoidRootPart.CFrame = myHrp.CFrame + (myHrp.CFrame.LookVector * 1.5)
		end
	end
	task.wait(0.1)
...
btnFechar.MouseButton1Click:Connect(function()
	puloInfinitoAtivo = false loopTpAtivo = false walkSpeedAtivo = false mmvAtivo = false espAtivo = false espArmaAtivo = false autoFacaAtivo = false
	if loopEsp then loopEsp:Disconnect() end
	if loopArma then loopArma:Disconnect() end
	limparESP()
	local char = localPlayer.Character
